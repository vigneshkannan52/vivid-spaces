...
header('Content-Disposition: attachment; filename=scheduled_appts_export.csv');
$output = fopen('php://output', 'w');

$export_columns = apply_filters( 'scheduled_csv_export_columns', array(
	'First Name',
	'Last Name',
	'Email',
	'Calendar',
...
if($scheduledAppointments->have_posts()):
	while ($scheduledAppointments->have_posts()):
		...
		if ($user_id):
			...
			if (!empty($user_info)):
				$customer_name = scheduled_get_name( $user_id, 'first' );
				$customer_surname = scheduled_get_name( $user_id, 'last' );
...
foreach($appointments_array as $appt_id => $appointment):
	$appointment = apply_filters( 'scheduled_csv_row_data', $appointment, $appt_id );
