...
function scheduled_is_timeslot_disabled( $date = false, $timeslot = false, $calendar_id = false ){
...
function scheduled_get_kb_article( $id ) {

	$kb_article = get_transient( 'scheduled_kb_article_' . $id );
	...
	if (empty($kb_article)):
		...
		set_transient( 'scheduled_kb_article_' . $id, $kb_article, 86400 );
	endif;
...
function scheduled_appts_available( $year = false, $month = false, $day = false, $calendar_id = false, $return_array = false, $include_past = false ){

	$prevent_before = apply_filters('scheduled_prevent_appointments_before',get_option('scheduled_prevent_appointments_before',false));
	$prevent_after = apply_filters('scheduled_prevent_appointments_after',get_option('scheduled_prevent_appointments_after',false));
	$buffer = get_option('scheduled_appointment_buffer',0);
	$buffer_string = apply_filters('scheduled_appointment_buffer_string','+'.$buffer.' hours');
	$disabled_timeslots = get_option( 'scheduled_disabled_timeslots',array() );
	...
	if($scheduledAppointments->have_posts()):
		while ($scheduledAppointments->have_posts()):
			...
			$appointments_scheduled[$year.$this_month.$this_day.'_'.$timeslot] = (isset($appointments_scheduled[$year.$this_month.$this_day.'_'.$timeslot]) ? $appointments_scheduled[$year.$this_month.$this_day.'_'.$timeslot] + 1 : 1);
		endwhile;
	endif;

	$appointments_scheduled = apply_filters('scheduled_appts_scheduled_array', $appointments_scheduled);
	...
	for($i = $first_day; $i <= $last_day; $i++){
		...
		if (isset($scheduled_defaults[$date_string]) && empty($scheduled_defaults[$date_string])):
			continue;
		endif;
		...
		if (isset($scheduled_defaults[$date_string]) && !empty($scheduled_defaults[$date_string])):
			...
			if (!is_array($scheduled_defaults[$date_string])):
				$scheduled_defaults[$date_string] = json_decode($scheduled_defaults[$date_string],true);
			endif;

			foreach($scheduled_defaults[$date_string] as $timeslot => $count):
				...
				if ( !$include_past || $include_past && $current_timestamp <= $this_timeslot_startstamp && apply_filters('scheduled_check_timeslot_startstamp',$this_timeslot_startstamp) ):
				...
				if (isset($appointments_scheduled[$date_string.'_'.$timeslot])):
					$available_timeslots = $available_timeslots - $appointments_scheduled[$date_string.'_'.$timeslot];
					$available_timeslots_array[$date_check][$timeslot] = ( isset($available_timeslots_array[$date_check][$timeslot]) ? $available_timeslots_array[$date_check][$timeslot] - $appointments_scheduled[$date_string.'_'.$timeslot] : 0 );
		...
		elseif (isset($scheduled_defaults[$day_name]) && !empty($scheduled_defaults[$day_name])):
			...
			if (!is_array($scheduled_defaults[$day_name])):
				$scheduled_defaults[$day_name] = json_decode($scheduled_defaults[$day_name],true);
			endif;

			foreach($scheduled_defaults[$day_name] as $timeslot => $count):
...
// Scheduled Front-End Calendar
function scheduled_fe_calendar($year = false,$month = false,$calendar_id = false,$force_calendar = false){

	do_action('scheduled_fe_calendar_before');
	...
	if (!$force_calendar):
		...
		do {
			...
			$appointments_available = scheduled_appts_available($year,$month,false,$calendar_id);
	...
	$appointments_array = apply_filters( 'scheduled_appts_date_array', $appointments_array );

	$hide_weekends = get_option('scheduled_hide_weekends',false);
	$hide_available_count = get_option('scheduled_hide_available_timeslots',false);
	$scheduled_pa_active = get_option('scheduled_public_appointments',false);

	// Appointments Array
	// [DAY] => [POST_ID] => [TIMESTAMP/STATUS]

	?><div class="scheduled-calendar<?php echo ($scheduled_pa_active ? ' scheduled-pa-active' : ''); ?>"<?php echo ($calendar_id ? ' data-calendar-id="'.$calendar_id.'"' : ''); ?><?php echo (!$force_calendar ? ' data-calendar-date="'.$currentMonth.'"' : ''); ?>>
		<div class="bc-head">
				...
				<div class="bc-col">
					...
					<span class="monthName">
						...
						<?php if ($monthShown != $currentMonth): ?>
							<a href="#" class="backToMonth" data-goto="<?php echo $currentMonth; ?>"><?php esc_html_e('Back to','scheduled'); ?> <?php echo date_i18n('F',strtotime($currentMonth)); ?></a>
						<?php endif; ?>
					</span>
			...
			for($i = 1; $i <= $rows; $i++){
				...
				for($x = 1; $x <= 7; $x++){
					...
					if (isset($scheduled_defaults[$formatted_date]) && !empty($scheduled_defaults[$formatted_date])):
						$full_count = (is_array($scheduled_defaults[$formatted_date]) ? $scheduled_defaults[$formatted_date] : json_decode($scheduled_defaults[$formatted_date],true));
					elseif (isset($scheduled_defaults[$formatted_date]) && empty($scheduled_defaults[$formatted_date])):
						$full_count = false;
					elseif (isset($scheduled_defaults[$day_name]) && !empty($scheduled_defaults[$day_name])):
						$full_count = $scheduled_defaults[$day_name];
					else :
						$full_count = false;
					endif;
					...
					if (isset($scheduled_defaults[$formatted_date]) && !is_array($scheduled_defaults[$formatted_date])):
						$scheduled_defaults[$formatted_date] = json_decode($scheduled_defaults[$formatted_date],true);
					endif;
					...
					if (isset($appointments_array[$year.$check_month.$date]) && !empty($appointments_array[$year.$check_month.$date])):
						foreach($appointments_array[$year.$check_month.$date] as $appt):
							if (isset($scheduled_defaults[$formatted_date][$appt['timeslot']])):
								$appointments_count++;
							elseif (!isset($scheduled_defaults[$formatted_date]) && isset($scheduled_defaults[$day_name]) && !empty($scheduled_defaults[$day_name]) && isset($scheduled_defaults[$day_name][$appt['timeslot']])):
					...
					$appointments_left = scheduled_appts_available($year,$check_month,$date,$calendar_id);

					if (!$appointments_left):
						if (!$scheduled_pa_active):
					...
					if ($hide_weekends && $day_of_week == 6 || $hide_weekends && $day_of_week == 7):
						...
						$html .= '<span class="date'.(!$hide_available_count && $appointments_left > 0 && !in_array('prev-date',$classes) && !in_array('blur',$classes) ? ' tooltipster" title="'.sprintf( _n('%d Available','%d Available',$appointments_left,'scheduled'),$appointments_left) : (!$hide_available_count && !$appointments_left && $scheduled_pa_active && !in_array('prev-date',$classes) && !in_array('blur',$classes) ? ' tooltipster" title="'.esc_html__('None Available','scheduled').'"' : '')).'"><span class="number">'. $date .'</span></span>';
						$html .= '</div>';

						$combined_date = $year.'-'.$check_month.'-'.$date;
						echo apply_filters('scheduled_fe_single_date',$html,$combined_date,$classes);
	...
	do_action('scheduled_fe_calendar_after');

}

function scheduled_fe_calendar_date_content($date,$calendar_id = false){

	do_action('scheduled_fe_calendar_date_before');

	$hide_unavailable_timeslots = get_option('scheduled_hide_unavailable_timeslots',false);
	$hide_available_count = get_option('scheduled_hide_available_timeslots',false);
	$public_appointments = get_option('scheduled_public_appointments',false);
	$total_available = 0;
	...
	echo '<div class="scheduled-appt-list">';
		...
		*/
		...
		$scheduledAppointments = new WP_Query( apply_filters('scheduled_fe_date_content_query',$args) );
		if($scheduledAppointments->have_posts()):
			while ($scheduledAppointments->have_posts()):
				$scheduledAppointments->the_post();
				global $post;
		...
		if (isset($scheduled_defaults[$formatted_date]) && !empty($scheduled_defaults[$formatted_date])):
			$todays_defaults = (is_array($scheduled_defaults[$formatted_date]) ? $scheduled_defaults[$formatted_date] : json_decode($scheduled_defaults[$formatted_date],true));
			$todays_defaults_details = (is_array($scheduled_defaults[$formatted_date.'-details']) ? $scheduled_defaults[$formatted_date.'-details'] : json_decode($scheduled_defaults[$formatted_date.'-details'],true));
		elseif (isset($scheduled_defaults[$formatted_date]) && empty($scheduled_defaults[$formatted_date])):
		...
		if ($todays_defaults){
			...
			foreach($todays_defaults as $timeslot => $count):
				...
				*/
				...
				$spots_available = apply_filters('scheduled_spots_available', $spots_available, $this_timeslot_timestamp);
				...
				if ($spots_available && $available || !$hide_unavailable_timeslots):
					...
					if ($timeslot_parts[0] == '0000' && $timeslot_parts[1] == '2400'):
						...
						$timeslotText = date_i18n($time_format,strtotime($timeslot_parts[0])) . (!get_option('scheduled_hide_end_times') ? ' &ndash; '.date_i18n($time_format,strtotime($timeslot_parts[1])) : '');
					endif;
					...
					if ($hide_unavailable_timeslots && !$available):
						...
						$button_text = (!$spots_available || !$available ? esc_html__('Unavailable','scheduled') : esc_html( _x('Book Appointment','Book a Single Appointment', 'scheduled') ) );
						$html = '<div class="timeslot scheduledClearFix'.($title && $only_titles == true ? ' scheduled-hide-time' : '').($hide_available_count || !$available ? ' timeslot-count-hidden' : '').(!$available ? ' timeslot-unavailable' : '').($title ? ' has-title ' : '').'">';

							$html .= '<span class="timeslot-time'.($public_appointments ? ' scheduled-public-appointments' : '').'">';

								$html .= apply_filters('scheduled_fe_calendar_timeslot_before','',$this_timeslot_timestamp,$timeslot,$calendar_id);
								...
								if (!$hide_available_count):
									$html .= '<span class="spots-available'.($spots_available == 0 ? ' empty' : '').'">';
										$html .= sprintf( _n( '%d space available', '%d spaces available', $spots_available, 'scheduled' ), $spots_available );
									$html .= '</span>';
								endif;

								if ($public_appointments && !empty($appts_in_this_timeslot)):
									$html .= '<span class="scheduled-public-appointment-title">'._n('Appointments in this time slot:','Appointments in this time slot:',count($appts_in_this_timeslot),'scheduled').'</span>';
									$html .= '<ul class="scheduled-public-appointment-list">';
									foreach($appts_in_this_timeslot as $appt_id):
										...
										if ($user_id):
											$html .= '<li>'.scheduled_get_name($user_id).($post_status != 'publish' ? ' <span class="scheduled-public-pending">(pending)</span>' : '').'</li>';
										elseif($guest_name):
											$html .= '<li>'.$guest_name.' '.$guest_surname.($post_status != 'publish' ? ' <span class="scheduled-public-pending">(pending)</span>' : '').'</li>';
										endif;
								...
								$html .= apply_filters('scheduled_fe_calendar_timeslot_after','',$this_timeslot_timestamp,$timeslot,$calendar_id);

							$html .= '</span>';
							$html .= '<span class="timeslot-people"><button data-calendar-id="'.$calendar_id.'" data-title="'.esc_attr($title).'" data-timeslot="'.$timeslot.'" data-date="'.$date.'" class="new-appt button"'.(!$spots_available || !$available ? ' disabled' : '').'>'.( $title ? '<span class="timeslot-mobile-title">'.esc_html($title).'</span>' : '' ).'<span class="button-timeslot">'.apply_filters('scheduled_fe_mobile_timeslot_button',$timeslotText,$this_timeslot_timestamp,$timeslot,$calendar_id).'</span>'.apply_filters('scheduled_button_book_appointment', '<span class="button-text">' . $button_text . '</span>' . ( !$hide_available_count ? '<span class="spots-available' . ( $spots_available == 0 ? ' empty' : '' ) . '">' . sprintf( esc_html( _n( '%d space available', '%d spaces available', $spots_available, 'scheduled' ) ), $spots_available ) . '</span>' : '' ) ).'</button></span>';
						$html .= '</div>';
					endif;

					echo apply_filters('scheduled_fe_calendar_date_appointments',$html,$time_format,$timeslot_parts,$spots_available,$available,$timeslot,$date);
			...
			if (!$temp_count):
				...
				echo '<p>'.esc_html__('There are no appointment time slots available for this day.','scheduled').'</p>';
		...
		*/
		...
		echo '<h2><span>' . sprintf( _n( esc_html(_x( 'Available Appointment on %s', 'Single Appointment', 'scheduled' )), esc_html(_x( 'Available Appointments on %s', 'Multiple Appointments', 'scheduled' )), $total_available ), '</span><strong>'.$date_display.'</strong><span>') . '</span></h2>';
		echo $appt_list_html;

	echo '</div>';

	do_action('scheduled_fe_calendar_date_after');

}

function scheduled_fe_appointment_list_content($date,$calendar_id = false,$force_day = false){
	...
	do {
		...
		$appointments_available = scheduled_appts_available($year,$month,$day,$calendar_id);
	...
	echo '<div class="scheduled-appt-list" data-list-date="'.$date.'" data-min-date="'.($prevent_before ? date_i18n('Y-m-d',strtotime($prevent_before)) : $earliest_date).'" data-max-date="'.($prevent_after ? date_i18n('Y-m-d',strtotime($prevent_after)) : false).'">';
		...
		*/
		...
		echo '<div class="scheduled-list-view-nav" data-calendar-id="'.$calendar_id.'">';
			if ($showing_prev):
				echo '<button data-date="'.date_i18n('Y-m-d',strtotime($prev_day)).'" class="scheduled-list-view-date-prev bb-small"><i class="fa-solid fa-arrow-left"></i>&nbsp;&nbsp;'.esc_html__('Previous','scheduled').'</button>';
			endif;
			if ($showing_next):
				echo '<button data-date="'.date_i18n('Y-m-d',strtotime($next_day)).'" class="scheduled-list-view-date-next bb-small">'.esc_html__('Next','scheduled').'&nbsp;&nbsp;<i class="fa-solid fa-arrow-right"></i></button>';
			endif;
			echo '<span class="scheduled-datepicker-wrap"><input data-min-date="'.$earliest_date.'" class="scheduled_list_date_picker" value="'.date_i18n('Y-m-d',strtotime($date)).'" type="hidden"><a href="#" class="scheduled_list_date_picker_trigger"><i class="fa-solid fa-calendar-days"></i></a></span>';
		echo '</div>';
		...
		*/
		...
		elseif (isset($scheduled_defaults[$day_name]) && !empty($scheduled_defaults[$day_name])):
			$todays_defaults = $scheduled_defaults[$day_name];
			$todays_defaults_details = isset( $scheduled_defaults[$day_name.'-details'] ) ? $scheduled_defaults[$day_name.'-details'] : false;
		else :
			$todays_defaults = false;
			$todays_defaults_details = false;
		endif;
					...
					if ($hide_unavailable_timeslots && !$available):
						...
						$html = '<div class="timeslot scheduledClearFix'.($title && $only_titles == true ? ' scheduled-hide-time' : '').($hide_available_count || !$available ? ' timeslot-count-hidden' : '').(!$available ? ' timeslot-unavailable' : '').($title ? ' has-title ' : '').'">';
							$html .= '<span class="timeslot-time'.($public_appointments ? ' scheduled-public-appointments' : '').'">';
								...
								$html .= '<span class="timeslot-range"><i class="fa-solid fa-clock"></i>&nbsp;&nbsp;' . $timeslotText . '</span>';
								if (!$hide_available_count): $html .= '<span class="spots-available'.($spots_available == 0 ? ' empty' : '').'">'.sprintf(_n('%d space available','%d spaces available',$spots_available,'scheduled'),$spots_available).'</span>'; endif;
								if ($public_appointments && !empty($appts_in_this_timeslot)):
									$html .= '<span class="scheduled-public-appointment-title">'._n('Appointments in this time slot:','Appointments in this time slot:',count($appts_in_this_timeslot),'scheduled').'</span>';
									$html .= '<ul class="scheduled-public-appointment-list">';
									foreach($appts_in_this_timeslot as $appt_id):
										...
										$user_id = get_post_meta($appt_id, '_appointment_user',true);
										$html .= '<li>'.scheduled_get_name($user_id).'</li>';
		...
		*/
		...
		echo '<div class="scheduled-appt-list-header">';
			echo '<h2'.(!$showing_prev ? ' class="scheduled-no-prev"' : '').'><span>'.sprintf(_n('Available Appointment on %s','Available Appointments on %s',$total_available,'scheduled'),'</span><strong>'.$date_display.'</strong>').'</h2>';
...
function scheduled_reset_password($user_login){
    ...
    $message = esc_html__('Someone requested that the password be reset for the following account:','scheduled') . "\r\n\r\n";
    $message .= network_home_url( '/' ) . "\r\n\r\n";
    $message .= sprintf(esc_html__('Username: %s','scheduled'), $user_login) . "\r\n\r\n";
    $message .= esc_html__('If this was a mistake, just ignore this email and nothing will happen.','scheduled') . "\r\n\r\n";
    $message .= esc_html__('To reset your password, visit the following address:','scheduled') . "\r\n\r\n";
    ...
    $title = sprintf( esc_html__('[%s] Password Reset','scheduled'), $blogname );
...
function scheduled_appt_is_available($date,$timeslot,$calendar_id = false){
...
function scheduled_custom_fields($calendar_id = false){

	if ($calendar_id):
		$custom_fields = json_decode(stripslashes(get_option('scheduled_custom_fields_'.$calendar_id)),true);
		if (empty($custom_fields)): $custom_fields = json_decode(stripslashes(get_option('scheduled_custom_fields')),true); endif;
	else:
		...
		foreach($custom_fields as $field):
			...
			if ($look_for_subs):
				...
				if ($field_type == 'single-checkbox'):
					...
					?><span class="checkbox-radio-block"><input <?php echo $data_attributes ?> type="checkbox" name="<?php echo $field['name']; ?>[]" id="scheduled-checkbox-<?php echo $field['name'].'-'.$temp_count; ?>" value="<?php echo htmlentities($field['value'], ENT_QUOTES | ENT_IGNORE, "UTF-8"); ?>"> <label for="scheduled-checkbox-<?php echo $field['name'].'-'.$temp_count; ?>"><?php echo $field['value']; ?></label></span><?php

				elseif ($field_type == 'single-radio-button'):

					?><span class="checkbox-radio-block"><input <?php echo $data_attributes ?> type="radio" name="<?php echo $field['name']; ?>[]" id="scheduled-radio-<?php echo $field['name'].'-'.$temp_count; ?>" value="<?php echo htmlentities($field['value'], ENT_QUOTES | ENT_IGNORE, "UTF-8"); ?>"> <label for="scheduled-radio-<?php echo $field['name'].'-'.$temp_count; ?>"><?php echo $field['value']; ?></label></span><?php
			...
			switch($field_type):
				...
				case 'single-line-text-label' :
					...
					?><div class="field">
						...
						<input id="scheduled-textfield-<?php echo esc_attr($field['name']); ?>" <?php echo $data_attributes ?> <?php if ($is_required): echo ' required="required"'; endif; ?> type="text" name="<?php echo esc_attr($field['name']); ?>" value="" class="large textfield" />
					</div><?php
				...
				case 'paragraph-text-label' :
					...
					?><div class="field">
						<label class="field-label"><?php echo $field['value']; ?><?php if ($is_required): ?><i class="required-asterisk fa-solid fa-asterisk"></i><?php endif; ?></label>
						<textarea id="scheduled-textarea-<?php echo esc_attr($field['name']); ?>" <?php echo $data_attributes ?> <?php if ($is_required): echo ' required="required"'; endif; ?> name="<?php echo esc_attr($field['name']); ?>"></textarea>
					</div><?php
				...
				case 'checkboxes-label' :
					...
					?><div class="field">
						<label class="field-label"><?php echo $field['value']; ?><?php if ($is_required): ?><i class="required-asterisk fa-solid fa-asterisk"></i><?php endif; ?></label>
						<input id="scheduled-checkbox-label-<?php echo esc_attr($field['name']); ?>" <?php echo $data_attributes ?> <?php if ($is_required): echo ' required="required"'; endif; ?> type="hidden" name="<?php echo esc_attr($field['name']); ?>" /><?php
				...
				case 'radio-buttons-label' :
					...
					?><div class="field">
						<label class="field-label"><?php echo $field['value']; ?><?php if ($is_required): ?><i class="required-asterisk fa-solid fa-asterisk"></i><?php endif; ?></label>
						<input id="scheduled-radio-label-<?php echo esc_attr($field['name']); ?>" <?php echo $data_attributes ?> <?php if ($is_required): echo ' required="required"'; endif; ?> type="hidden" name="<?php echo esc_attr($field['name']); ?>" /><?php
				...
				case 'drop-down-label' :
					...
					?><div class="field">
						<label class="field-label"><?php echo $field['value']; ?><?php if ($is_required): ?><i class="required-asterisk fa-solid fa-asterisk"></i><?php endif; ?></label>
						<input id="scheduled-select-label-<?php echo esc_attr($field['name']); ?>" type="hidden" name="<?php echo esc_attr($field['name']); ?>" />
						<select id="scheduled-select-<?php echo esc_attr($field['name']); ?>" <?php echo $data_attributes ?> <?php if ($is_required): echo ' required="required"'; endif; ?> name="<?php echo esc_attr($field['name']); ?>"><option value=""><?php esc_html_e('Choose...','scheduled'); ?></option><?php
				...
				case 'plain-text-content' :
					...
					?><div id="scheduled-text-<?php echo $field['name']; ?>" class="field scheduled-text-content">
...
function scheduled_hidden_login_field(){
	$additional_field = '<input type="hidden" name="scheduled_login_form" value="1">';
	return $additional_field;
}

add_action( 'wp_login_failed', 'scheduled_fe_login_fail' );  // hook failed login
function scheduled_fe_login_fail( $username ) {
	if ( isset( $_SERVER['HTTP_REFERER'] ) && isset( $_POST['scheduled_login_form'] ) ):
...
function scheduled_send_user_approved_email($appt_id){
	...
	if ($email_content && $email_subject):
		...
		do_action( 'scheduled_approved_email', $token_replacements['email'], $email_subject, $email_content );

	endif;

}

function scheduled_user_appointments($user_id,$only_count = false,$time_format = false,$date_format = false,$historic = false){
	...
	if($scheduledAppointments->have_posts()):
		while ($scheduledAppointments->have_posts()):
			...
			$calendar_id = wp_get_post_terms( $post->ID, 'scheduled_custom_calendars' );
...
function scheduled_profile_update_submit(){
		...
		if ( 'POST' == $_SERVER['REQUEST_METHOD'] && !empty( $_POST['action'] ) && $_POST['action'] == 'update-user' ) {
		    ...
		    if (isset($_POST['pass1']) && isset($_POST['pass2']) && $_POST['pass1'] && $_POST['pass2'] ) {
		        if ( $_POST['pass1'] == $_POST['pass2'] )
		            wp_update_user( array( 'ID' => $scheduled_current_user->ID, 'user_pass' => esc_attr( $_POST['pass1'] ) ) );
		        else
		    ...
		    if ( isset( $_POST['url'] ) )
		    	wp_update_user( array( 'ID' => $scheduled_current_user->ID, 'user_url' => esc_url( $_POST['url'] ) ) );
		    if ( isset( $_POST['email'] ) ){
		        ...
		        elseif( $email_exists && $email_exists != $scheduled_current_user->ID )
		            $error[] = esc_html__('This email is already used by another user.  try a different one.', 'profile');
		        else{
		            wp_update_user( array ('ID' => $scheduled_current_user->ID, 'user_email' => esc_attr( $_POST['email'] )));
		        }
		    }

		    if ( isset( $_POST['nickname'] ) ):
		        update_user_meta( $scheduled_current_user->ID, 'nickname', esc_attr( $_POST['nickname'] ) );
		        wp_update_user( array ('ID' => $scheduled_current_user->ID, 'display_name' => esc_attr( $_POST['nickname'] )));
		    endif;
	        ...
	        if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] == 0):
			    ...
			    if (isset($avatar,$_POST['avatar_nonce']) && $avatar && wp_verify_nonce( $_POST['avatar_nonce'], 'avatar_upload' )) {
				    ...
				    if ( is_wp_error( $attachment_id ) ) {
				        $error[] = esc_html__('Error uploading avatar.','scheduled');
				    } else {
				        update_user_meta( $scheduled_current_user->ID, 'avatar', $attachment_id );
				    }

			    } else {

			    	$error[] = esc_html__('Avatar uploader security check failed.','scheduled');
		    ...
		    if ( count($error) == 0 ) {
		        ...
		        do_action('edit_user_profile_update', $scheduled_current_user->ID);
	...
	if ( apply_filters( 'scheduled_sessions_enabled', true ) ){
		if (isset($_SESSION['appt_requested']) && isset($_SESSION['new_account'])){
			...
			if ($appointment_default_status == 'draft'):
				echo '<p class="scheduled-form-notice">'.esc_html__('Your appointment has been requested! It will be updated below if approved.','scheduled').'</p>';
			else :
				echo '<p class="scheduled-form-notice">'.esc_html__('Your appointment has been added to our calendar!','scheduled').'</p>';
			endif;

		}
	}

	echo do_shortcode('[scheduled-appointments remove_wrapper=1]');

}

function scheduled_profile_content_history(){

	echo do_shortcode('[scheduled-appointments remove_wrapper=1 historic=1]');

}

function scheduled_profile_content_edit(){

	$scheduled_current_user = wp_get_current_user();

	echo '<h4><i class="fa-solid fa-pen-to-square" style="position:relative; top:-1px;"></i>&nbsp;&nbsp;'.esc_html__('Edit Profile','scheduled').'</h4>'; ?>

    <form method="post" enctype="multipart/form-data" id="scheduled-page-form" action="<?php the_permalink(); ?>">

	    <div class="scheduledClearFix">
            <p class="form-avatar">
                <label for="avatar"><?php esc_html_e('Update Avatar', 'scheduled'); ?><?php if (SCHEDULED_DEMO_MODE): ?> <span class="not-bold"><?php esc_html_e('(disabled in demo)', 'cooked'); ?></span><?php endif; ?></label><br>
                <span class="scheduled-upload-wrap"><span><?php esc_html_e('Choose image ...','scheduled'); ?></span><input<?php if (SCHEDULED_DEMO_MODE): ?> disabled<?php endif; ?> class="field" name="avatar" type="file" id="avatar" value="" /></span>
                <?php wp_nonce_field( 'avatar_upload', 'avatar_nonce' ); ?>
                <span class="hint-p"><?php esc_html_e('Recommended size: 100px by 100px or larger', 'scheduled'); ?></span>
            </p><!-- .form-nickname -->
    	</div>

        <div class="scheduledClearFix">
            <p class="form-nickname">
                <label for="nickname"><?php esc_html_e('Display Name', 'scheduled'); ?><?php if (SCHEDULED_DEMO_MODE): ?> <span class="not-bold"><?php esc_html_e('(disabled in demo)', 'cooked'); ?></span><?php endif; ?></label>
                <input<?php if (SCHEDULED_DEMO_MODE): ?> disabled<?php endif; ?> class="text-input" name="nickname" type="text" id="nickname" value="<?php the_author_meta( 'nickname', $scheduled_current_user->ID ); ?>" />
            </p><!-- .form-nickname -->
            <p class="form-email">
                <label for="email"><?php esc_html_e('E-mail *', 'scheduled'); ?><?php if (SCHEDULED_DEMO_MODE): ?> <span class="not-bold"><?php esc_html_e('(disabled in demo)', 'cooked'); ?></span><?php endif; ?></label>
                <input<?php if (SCHEDULED_DEMO_MODE): ?> disabled<?php endif; ?> class="text-input" name="email" type="text" id="email" value="<?php the_author_meta( 'user_email', $scheduled_current_user->ID ); ?>" />
            </p><!-- .form-email -->
        </div>
        <div class="scheduledClearFix">
            <p class="form-password">
                <label for="pass1"><?php esc_html_e('Change Password', 'scheduled'); ?><?php if (SCHEDULED_DEMO_MODE): ?> <span class="not-bold"><?php esc_html_e('(disabled in demo)', 'cooked'); ?></span><?php endif; ?></label>
                <input<?php if (SCHEDULED_DEMO_MODE): ?> disabled<?php endif; ?> class="text-input" name="pass1" type="password" id="pass1" />
            </p><!-- .form-password -->
            <p class="form-password last">
                <label for="pass2"><?php esc_html_e('Repeat Password', 'scheduled'); ?><?php if (SCHEDULED_DEMO_MODE): ?> <span class="not-bold"><?php esc_html_e('(disabled in demo)', 'cooked'); ?></span><?php endif; ?></label>
                <input<?php if (SCHEDULED_DEMO_MODE): ?> disabled<?php endif; ?> class="text-input" name="pass2" type="password" id="pass2" />
            </p><!-- .form-password -->
        </div>

        <?php
            //action hook for plugin and extra fields
            do_action('edit_user_profile',$scheduled_current_user);
        ?>
        <p class="form-submit">
            <input name="updateuser" type="submit" id="updateuser" class="submit button button-primary" value="<?php esc_html_e('Update', 'scheduled'); ?>" />
