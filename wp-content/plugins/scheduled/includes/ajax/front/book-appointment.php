...
do_action('scheduled_before_creating_appointment');
...
if ( !$is_new_registration && $date && $timeslot && isset($calendar_id_for_cf) ):
	...
	$appt_is_available = scheduled_appt_is_available($date,$timeslot,$calendar_id_for_cf);
...
if ($appt_is_available):
	...
	if ( $calendar_id_for_cf ) {
		$custom_fields = json_decode(stripslashes(get_option('scheduled_custom_fields_'.$calendar_id_for_cf)),true);
	}
	...
	if (!empty($custom_fields)):
		...
		$custom_field_data = apply_filters('scheduled_custom_field_data', $custom_field_data);
	...
	if ($customer_type == 'guest'):
		...
		if ( $name_requirements == 'require_surname' && !$surname ):
			...
			echo 'error###'.esc_html__('Your full name is required to book an appointment.','scheduled');
			...
			if ($email && is_email($email) && $name || !$email && !$email_required && $name):
				...
				if ( apply_filters('scheduled_update_cf_meta_value', true) ) {
					update_post_meta($post_id, '_cf_meta_value', $cf_meta_value);
				}

				if ( apply_filters('scheduled_update_appointment_calendar', true) ) {
					if (!empty($calendar_id)): $calendar_term = get_term_by('id',$calendar_id[0],'scheduled_custom_calendars'); $calendar_name = $calendar_term->name; wp_set_object_terms($post_id,$calendar_id,'scheduled_custom_calendars'); else: $calendar_name = false; endif;
				}

				// Send a confirmation email to the User?
				$email_content = get_option('scheduled_appt_confirmation_email_content',false);
				$email_subject = get_option('scheduled_appt_confirmation_email_subject',false);

				$token_replacements = scheduled_get_appointment_tokens( $post_id );

				if ( $email_content && $email_subject ):

					$admin_email = scheduled_which_admin_to_send_email( esc_html( $_POST['calendar_id'] ) );
					...
					do_action( 'scheduled_confirmation_email', $email, $email_subject, $email_content, $admin_email );

				endif;

				// Send an email to the Admin?
				$email_content = get_option('scheduled_admin_appointment_email_content',false);
				$email_subject = get_option('scheduled_admin_appointment_email_subject',false);
				if ($email_content && $email_subject):

					$admin_email = scheduled_which_admin_to_send_email( esc_html( $_POST['calendar_id'] ) );
					$email_content = scheduled_token_replacement( $email_content,$token_replacements );
					$email_subject = scheduled_token_replacement( $email_subject,$token_replacements );
					do_action( 'scheduled_admin_confirmation_email', $admin_email, $email_subject, $email_content, $token_replacements['email'], $token_replacements['name'] );
				...
				if ($email && !is_email($email)):
					$errors[] = esc_html__('The email address you have entered doesn\'t appear to be valid.','scheduled');
				elseif ($email_required && !$email):
					$errors[] = esc_html__('Your name and a valid email address are required to book an appointment.','scheduled');
				elseif (!$name):
					$errors[] = esc_html__('Your name is required to book an appointment.','scheduled');
				else:
					$errors[] = esc_html__('An unknown error occured.','scheduled');
				endif;
...
',$errors);
	...
	elseif ($customer_type == 'current'):
		...
		// Send a confirmation email to the User?
		$email_content = get_option('scheduled_appt_confirmation_email_content');
		$email_subject = get_option('scheduled_appt_confirmation_email_subject');

		$token_replacements = scheduled_get_appointment_tokens( $post_id );

		if ($email_content && $email_subject):

			$admin_email = scheduled_which_admin_to_send_email($_POST['calendar_id']);
			$email_content = scheduled_token_replacement( $email_content,$token_replacements );
			$email_subject = scheduled_token_replacement( $email_subject,$token_replacements );

			do_action( 'scheduled_confirmation_email', $token_replacements['email'], $email_subject, $email_content, $admin_email );

		endif;

		// Send an email to the Admin?
		$email_content = get_option('scheduled_admin_appointment_email_content');
		$email_subject = get_option('scheduled_admin_appointment_email_subject');
	...
	elseif ($customer_type == 'new'):
		...
		$name = esc_attr($_POST['scheduled_appt_name']);
		$surname = ( isset($_POST['scheduled_appt_surname']) && $_POST['scheduled_appt_surname'] ? esc_attr($_POST['scheduled_appt_surname']) : false );
		$fullname = ( $surname ? $name . ' ' . $surname : $name );
		$email = $_POST['scheduled_appt_email'];
		$password = $_POST['scheduled_appt_password'];
		...
		if ( $name_requirements == 'require_surname' && !$surname ):
			...
			$errors = scheduled_registration_validation($email,$password,$captcha_word,$captcha_code);
			...
			if (empty($errors)):
				...
				if (apply_filters('scheduled_update_appointment_calendar', true)) {
					if (!empty($calendar_id)): wp_set_object_terms($post_id,$calendar_id,'scheduled_custom_calendars'); endif;
				}
				...
				if ($email_content && $email_subject):
					...
					$admin_email = scheduled_which_admin_to_send_email( $email_calendar_id );
					$email_content = scheduled_token_replacement( $email_content,$token_replacements );
					$email_subject = scheduled_token_replacement( $email_subject,$token_replacements );
				...
				if ($email_content && $email_subject):
					...
					$admin_email = scheduled_which_admin_to_send_email( esc_html( $_POST['calendar_id'] ) );
					$email_content = scheduled_token_replacement( $email_content,$registration_token_replacements,'user' );
					$email_subject = scheduled_token_replacement( $email_subject,$registration_token_replacements,'user' );

					do_action( 'scheduled_registration_email', $registration_token_replacements['email'], $email_subject, $email_content, $admin_email );
	...
	if ( $name_requirements == 'require_surname' && !$surname ):
		...
		if (empty($errors)):
			...
			if ($email_content && $email_subject):
				...
				$email_subject = scheduled_token_replacement( $email_subject,$token_replacements,'user' );

				do_action( 'scheduled_registration_email', $token_replacements['email'], $email_subject, $email_content );
	        ...
	        echo 'success###' . esc_html__('Registration has been successful.','scheduled');
...
',$errors);
	...
	$error_message = apply_filters(
		'scheduled_availability_error_message',
		esc_html__('Sorry, someone just scheduled this appointment before you could. Please choose a different booking time.','scheduled')
	);
