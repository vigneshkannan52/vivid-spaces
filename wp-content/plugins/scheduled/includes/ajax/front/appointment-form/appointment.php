...
$appt_date_time_before = apply_filters('scheduled_fe_appt_form_date_time_before', '', $this_timeslot_timestamp, $timeslot, $calendar_id );
$appt_date_time_after = apply_filters( 'scheduled_fe_appt_form_date_time_after', '', $this_timeslot_timestamp, $timeslot, $calendar_id );

$date_format = get_option('date_format');
$time_format = get_option('time_format');

$only_titles = get_option('scheduled_show_only_titles', false);
$hide_end_titles = get_option('scheduled_hide_end_times');
$all_day_text = esc_html__( 'All day', 'scheduled' );
...
if (!empty($calendar_id)): $calendar_term = get_term_by('id',$calendar_id,'scheduled_custom_calendars'); $calendar_name = '<p class="calendar-name">' . $calendar_term->name . '</p>'; else: $calendar_name = false; endif;
...
?><div class="scheduled-appointment-details" data-appt-key="<?php echo $appointment_key; ?>"><?php
	...
	if ( $appt_timeslot == $all_day_text ):
		?><p class="appointment-info"><i class="fa-solid fa-calendar-days"></i>&nbsp;&nbsp;&nbsp;<?php echo sprintf( esc_html__( '%s on %s','scheduled' ), $appt_timeslot, $appt_date_name ); ?></p><?php
	else:
		if ( get_option('scheduled_show_only_titles') && $appt_title ):
			?><p class="appointment-info"><i class="fa-solid fa-calendar-days"></i>&nbsp;&nbsp;&nbsp;<?php echo $appt_date_name; ?></p><?php
		else:
			?><p class="appointment-info"><i class="fa-solid fa-calendar-days"></i>&nbsp;&nbsp;&nbsp;<?php echo sprintf( esc_html__( '%s at %s','scheduled' ), $appt_date_name, $appt_timeslot ); ?></p><?php
		endif;
	endif;
