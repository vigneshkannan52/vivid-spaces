...
$scheduled_current_user = $is_user_logged_in ? wp_get_current_user() : false;

$guest_booking = get_option( 'scheduled_booking_type', 'registered' ) === 'guest';
$new_appointment_default = get_option('scheduled_new_appointment_default','draft');
...
$appointment_limit = get_option( 'scheduled_appointment_limit' );
if ( $is_user_logged_in && $appointment_limit ) {
	$upcoming_user_appointments = scheduled_user_appointments( $scheduled_current_user->ID, true );
...
<?php if ( ! $is_user_logged_in && ! $guest_booking ): ?>
	...
	<form name="customerChoices" action="" id="customerChoices" class="scheduledClearFix"<?php echo ( !get_option('users_can_register') ? ' style="display:none;"' : '' ); ?>>
		...
		<?php if ( get_option('users_can_register') ): ?>
			<div class="field">
				<span class="checkbox-radio-block">
					...
					<label for="customer_new"><?php esc_html_e('New customer','scheduled'); ?></label>
				</span>
			</div>
		<?php endif; ?>
		...
		<div class="field">
			<span class="checkbox-radio-block">
				...
				<label for="customer_current"><?php esc_html_e('Current customer','scheduled'); ?></label>
			</span>
		</div>
	</form>
	...
	<div class="condition-block customer_choice<?php echo ( !get_option('users_can_register') && !is_user_logged_in() ? ' default' : '' ); ?>" id="condition-current">
		...
		<form id="ajaxlogin" action="" method="post" data-date="<?php echo $first_booking['date']; ?>" data-title="<?php echo $first_booking['title']; ?>" data-timeslot="<?php echo $first_booking['timeslot']; ?>" data-calendar-id="<?php echo $first_booking['calendar_id']; ?>">
			<div class="cf-block">
				...
				<?php include(SCHEDULED_AJAX_INCLUDES_DIR . 'front/appointment-form/form-fields-login.php'); ?>

				<input type="hidden" name="action" value="scheduled_ajax_login">
				...
				<?php if ( !is_multisite() ): ?>
				<a href="#" class="scheduled-forgot-password"><?php esc_html_e( 'I forgot my password.', 'scheduled' ); ?></a>
				<?php endif; ?>

			</div>

			<div class="field">
				<input name="submit" type="submit" class="button button-primary" value="<?php esc_html_e('Sign in', 'scheduled') ?>">
		...
		<?php if ( !is_multisite() ): ?>
			<form id="ajaxforgot" action="" method="post">
				<div class="cf-block" style="margin:0 0 5px;">
					...
					<?php include(SCHEDULED_AJAX_INCLUDES_DIR . 'front/appointment-form/form-fields-forgot.php'); ?>

					<input type="hidden" name="action" value="scheduled_ajax_forgot">
				...
				<div class="field">
					<input name="submit_forgot" type="submit" class="button button-primary" value="<?php esc_html_e('Reset Password', 'scheduled') ?>">
					<button class="scheduled-forgot-goback button"><?php esc_html_e('Go Back','scheduled'); ?></button>
				</div>
			</form>
		<?php endif; ?>
...
<div class="condition-block customer_choice<?php echo ( $guest_booking || get_option('users_can_register') && !is_user_logged_in() || is_user_logged_in() ? ' default' : '' ); ?>" id="condition-new">
	<form action="" method="post" id="newAppointmentForm">
		...
		<input type="hidden" name="action" value="scheduled_add_appt" />

		<?php if ( $is_user_logged_in ): ?>
			<input type="hidden" name="user_id" value="<?php echo $scheduled_current_user->ID; ?>" />
		<?php endif ?>
		...
		// User limit reached
		if ( $reached_limit ) {
			$error_message = sprintf(_n("Sorry, but you've hit the appointment limit. Each user may only book %d appointment at a time.","Sorry, but you've hit the appointment limit. Each user may only book %d appointments at a time.", $appointment_limit, "scheduled" ), $appointment_limit);
		}
		...
		if ( $will_reached_limit && ! $reached_limit ) {
			$error_message = sprintf(esc_html__("Sorry, but you're about to book more appointments than you are allowed to book at a time. Each user may only book %d appointments at a time.", "scheduled" ), $appointment_limit);
		}
		...
		if ( $is_user_logged_in && ! $error_message ) {
			$msg = sprintf( _n( 'You are about to request an appointment for %s.', 'You are about to request appointments for %s.', $total_appts, 'scheduled' ), '<em>' . scheduled_get_name( $scheduled_current_user->ID ) . '</em>' ) . ' ' . _n( 'Please review and confirm that you would like to request the following appointment:', 'Please review and confirm that you would like to request the following appointments:', $total_appts, 'scheduled' );
			echo wpautop( $msg );
		}

		// If there aren't any errors, and the user isn't logged in
		if ( ! $is_user_logged_in && ! $error_message ) {
			$msg = _n( 'Please confirm that you would like to request the following appointment:', 'Please confirm that you would like to request the following appointments:', $total_appts, 'scheduled' );
			echo wpautop( $msg );
		}
		...
		if ( ! $error_message ) {
			...
			include( SCHEDULED_AJAX_INCLUDES_DIR . 'front/appointment-form/bookings.php' );
		}
		...
		if ( ! $is_user_logged_in && ! $error_message && class_exists('ReallySimpleCaptcha') ) : ?>
			...
			?>
			<p class="captcha">
				<label for="captcha_code"><?php esc_html_e('Please enter the following text:','scheduled'); ?></label>
		...
		<div class="field">
			<?php if ( $error_message ): ?>
				<button class="cancel button"><?php esc_html_e('Okay','scheduled'); ?></button>
			<?php else: ?>
				<input type="submit" id="submit-request-appointment" class="button button-primary" value="<?php echo ( $new_appointment_default == 'draft' ? esc_html( _n( 'Request Appointment', 'Request Appointments', $total_appts, 'scheduled' ) ) : esc_html( _n( 'Book Appointment', 'Book Appointments', $total_appts, 'scheduled' ) ) ); ?>">
				<button class="cancel button"><?php esc_html_e('Cancel','scheduled'); ?></button>
			<?php endif; ?>
		</div>
	</form>
