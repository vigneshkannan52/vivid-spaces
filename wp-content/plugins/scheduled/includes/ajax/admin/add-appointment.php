...
if ($customer_type == 'guest'):
	...
	if ( $email_required && $email && is_email($email) && $name || !$email_required && !$email && $name):
		...
		if (apply_filters('scheduled_update_appointment_calendar', true)) {
			if (isset($calendar_id) && $calendar_id): wp_set_object_terms($post_id,$calendar_id,'scheduled_custom_calendars'); endif;
		}
		...
		if ($email && $email_content && $email_subject):
			...
			do_action( 'scheduled_approved_email', $email, $email_subject, $email_content );
...
$new_post = apply_filters('scheduled_new_appointment_args', array(
	...
	$errors = scheduled_registration_validation($email,$password);
