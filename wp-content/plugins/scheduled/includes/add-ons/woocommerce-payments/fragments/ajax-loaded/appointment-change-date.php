...
<div class="scheduled-form scheduled-scrollable">
	...
	<p class="scheduled-title-bar"><small><?php echo __('Update Appointment Date', 'scheduled'); ?></small></p>
	...
	<form action="" method="post" id="newAppointmentForm" data-calendar-id="<?php echo $calendar_id; ?>">
		...
		<p><?php echo sprintf(__('Please confirm that you would like to change the appointment date for %s to the following:', 'scheduled'), $user_nickname); ?></p>

		<div class="scheduled-appointments">
			<div class="scheduled-appointment-details">
				<p class="appointment-info"><i class="fa-solid fa-calendar-days"></i>&nbsp;&nbsp;&nbsp;<?php echo sprintf( esc_html__( '%s at %s','scheduled' ), $appt_date_name, $appt_timeslot ); ?></p>
			</div>
		</div>
		...
		<div class="field">
			<?php if (!$reached_limit): ?>
				<input type="submit" id="submit-edit-request-appointment" class="button button-primary" value="<?php _e('Update Appointment Date', 'scheduled'); ?>">
				<button class="cancel button"><?php _e('Cancel', 'scheduled'); ?></button>
			<?php else: ?>
				<button class="cancel button"><?php _e('Okay', 'scheduled'); ?></button>
			<?php endif; ?>
		</div>
