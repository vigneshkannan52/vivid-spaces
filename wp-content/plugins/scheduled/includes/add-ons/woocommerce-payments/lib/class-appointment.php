...
class Scheduled_WC_Appointment {
	...
	public static function get( $post_id=null ) {
		if ( !is_integer($post_id) ) {
			$message = sprintf( __('%s integer expected when %s given.', 'scheduled'), 'Scheduled_WC_Appointment::get($post_id)', gettype($post_id) );
	...
	protected function get_timeslot_text() {
		...
		if ( !empty($this->timeslot) ):
			...
			$hide_end_times = get_option('scheduled_hide_end_times');
			...
			$show_only_titles = get_option('scheduled_show_only_titles');
			if ( $show_only_titles && $this->title ){
				...
				if ($timeslots[0] == '0000' && $timeslots[1] == '2400') {
					$timeslotText = $day_year . ' (' . esc_html__('All day','scheduled') . ')';
				} else if ( !$hide_end_times ) {
					$timeslotText = sprintf(__('from %1$s to %2$s on %3$s', 'scheduled'), $time_start, $time_end, $day_year);
				} else if ( $hide_end_times ) {
					$timeslotText = sprintf(__('at %1$s on %2$s', 'scheduled'), $time_start, $day_year);
				} else {
					$timeslotText = 'N/A';
				}
	...
	protected function get_calendar() {
		$calendars = wp_get_post_terms($this->post_id, SCHEDULED_WC_TAX_CALENDAR);

		if ( !is_wp_error($calendars) && isset($calendars[0]) ) {
			$calendar_obj = $calendars[0];
			$calendar_page = Scheduled_WC_Functions::get_calendar_page_info($calendar_obj->term_id);
		} else {
			$calendar_obj = false;
			$calendar_page = Scheduled_WC_Functions::get_calendar_page_info();
		}
	...
	protected function get_order() {
		$this->order_id = get_post_meta($this->post_id, '_' . SCHEDULED_WC_PLUGIN_PREFIX . 'appointment_order_id', true);
		return $this;
	}

	protected function get_payment_status() {
		$payment_status = new Scheduled_WC_Appointment_Payment_Status($this->post_id);
