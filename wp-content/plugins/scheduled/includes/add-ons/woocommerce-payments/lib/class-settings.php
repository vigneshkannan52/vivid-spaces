...
add_filter('scheduled_admin_scheduled_screens', array('Scheduled_WC_Settings_Hooks', 'scheduled_admin_scheduled_screens'), 10, 1);

class Scheduled_WC_Settings {
	...
	protected function _setup() {
		add_action('admin_init', array('Scheduled_WC_Settings_Fields', 'register') );
	}
	...
	public function render_payment_options_page() {
		Scheduled_WC_Fragments::load('settings', 'payment-options');
	}

	public static function get_option($option_name) {
		$options = get_option(SCHEDULED_WC_PLUGIN_PREFIX . 'payment_options');
...
class Scheduled_WC_Settings_Hooks {

	public static function scheduled_admin_scheduled_screens( $screens ) {
		$screens[] = SCHEDULED_WC_PLUGIN_PREFIX . 'payment_options';

		return $screens;
	}
}

class Scheduled_WC_Settings_Fields {

	protected $options_name;

	private function __construct() {
		$this->options_name = SCHEDULED_WC_PLUGIN_PREFIX . 'payment_options';
	...
	protected function register_fields() {
		...
		add_settings_field(
			'email_confirmations',
			__('Email Confirmations', 'scheduled'),
		...
		add_settings_field(
			'enable_thumbnails',
			__('Enable Thumbnails', 'scheduled'),
		...
		add_settings_field(
			'enable_auto_cleanup',
			__('Enable Auto-Cleanup', 'scheduled'),
		...
		add_settings_field(
			'redirect_page',
			__('Redirect After Booking', 'scheduled'),
	...
	public function email_confirmations() {
		...
		$radio_options = array(
			'after_complete' => __('Send Scheduled confirmation emails ONLY after payment is marked as "complete".', 'scheduled'),
			'before_complete' => __('Send Scheduled confirmation emails right away.', 'scheduled')
		);
		?>
		<p><?php echo __('When do you want Scheduled to send out its confirmation emails?', 'scheduled') ?></p>
	...
	public function enable_thumbnails() {
		...
		$radio_options = array(
			'enable' => __('Enable', 'scheduled'),
			'disable' => __('Disable', 'scheduled')
		);
		?>
		<p><?php echo __('Enable this to show product thumbnails for Scheduled items (on the cart/checkout screens).', 'scheduled') ?></p>
		<?php foreach ($radio_options as $option_value => $option_label): ?>
			<div>
	...
	public function redirect_page() {
		...
		$radio_options = array(
			'checkout' => __('Checkout Page (default)', 'scheduled'),
			'cart' => __('Cart Page', 'scheduled')
		);
		?>
		<p><?php echo __('You can choose to have Scheduled redirect to the Checkout page (default) or the Cart page instead.', 'scheduled') ?></p>
		<?php foreach ($radio_options as $option_value => $option_label): ?>
			<div>
	...
	public function field_modes() {
		...
		usort($schedules, array($this, 'scheduled_schedule_sort'));

		?>
		<p><?php echo __('More schedule intervals can be added with a plugin like <a href="https://wordpress.org/plugins/wp-crontrol/" target="_blank">WP Crontrol</a>.', 'scheduled') ?> <?php echo __('The currently available schedules are listed below.', 'scheduled') ?></p>
	...
	public function scheduled_schedule_sort($a, $b) {
