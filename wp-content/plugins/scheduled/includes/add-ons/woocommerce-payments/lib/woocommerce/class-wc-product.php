...
class Scheduled_WC_Product {
	...
	public static function get( $post_id = null ) {
		...
		if ( !is_integer( $post_id ) ) {
			$message = sprintf( __('%s integer expected when %s given.', 'scheduled'), 'Scheduled_WC_Product::get($post_id)', gettype($post_id) );
	...
	protected function _get_product_data() {
		...
		if ( !$this->data ) {
			//$message = sprintf(__('An error has occur while retrieving product data for product with ID %1$d.', 'scheduled'), $this->post_id);
			//throw new Exception($message);
		}
	...
	protected function _get_title() {
		...
		$scheduled_wc_currency_symbol = get_woocommerce_currency_symbol();
		$scheduled_wc_currency_position = get_option( 'woocommerce_currency_pos','left' );
		...
		if ( $this->type === 'variable' ) {
			...
			switch ( $scheduled_wc_currency_position ) {
				case 'left' :
					$this->title = $scheduled_wc_currency_symbol . $this_price . ' - ' . $this->data->get_name();
				break;
				case 'right' :
				 	$this->title = $this_price . $scheduled_wc_currency_symbol . ' - ' . $this->data->get_name();
				break;
				case 'left_space' :
				  	$this->title = $scheduled_wc_currency_symbol . ' ' . $this_price . ' - ' . $this->data->get_name();
				break;
	...
	protected function _get_variations() {
		if ( $this->type==='variable' ) {
			...
			add_filter('woocommerce_available_variation', array('Scheduled_WC_Variation', 'woocommerce_available_variation'), 10, 3);
			$product_variations = $this->data->get_available_variations();
			remove_filter('woocommerce_available_variation', array('Scheduled_WC_Variation', 'woocommerce_available_variation'));
