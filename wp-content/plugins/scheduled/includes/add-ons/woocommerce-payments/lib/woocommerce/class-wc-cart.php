...
class Scheduled_WC_Cart {
	...
	public static function add_appointment( $app_id=null ) {
		...
		if ( !$appointment->products ) {
			$message = sprintf(__('Appointment with ID %1$d does not have any products assigned to it.', 'scheduled'), $post_id);
		...
		foreach ($appointment->products as $product) {
			...
			$additional_item_data = array(
				SCHEDULED_WC_PLUGIN_PREFIX . 'appointment_id' => $app_id
			);

			// add calendar name as part of the item data
			if ( !empty($appointment->calendar->calendar_obj) ) {
				$additional_item_data[SCHEDULED_WC_PLUGIN_PREFIX . 'appointment_cal_name'] = $appointment->calendar->calendar_obj->name;
			}
			// <---
			...
			if ( !empty($appointment->calendar->calendar_obj) ) {
				...
				if ( $assignee_email && ($usr=get_user_by('email', $assignee_email)) ) {
					$additional_item_data[SCHEDULED_WC_PLUGIN_PREFIX . 'appointment_assignee_name'] = $usr->display_name;
				}
			}
			// <---

			// add timerange name as part of the item data
			$additional_item_data[SCHEDULED_WC_PLUGIN_PREFIX . 'appointment_timerange'] = $appointment->timeslot_text;
			// <---
	...
	public static function get_cart_appointments() {
		...
		if ( method_exists($cart, 'get_cart') ):
			...
			$app_id_key = SCHEDULED_WC_PLUGIN_PREFIX . 'appointment_id';
...
class Scheduled_WC_Cart_Hooks {

	public static function woocommerce_checkout_fields( $fields ) {
	    global $woocommerce;
	    $scheduled_first_name = $woocommerce->session->get( 'scheduled_first_name' );
	    $scheduled_last_name = $woocommerce->session->get( 'scheduled_last_name' );
	    $scheduled_email = $woocommerce->session->get( 'scheduled_email' );

	    if(!is_null($scheduled_first_name)):
	    	$fields['billing']['billing_first_name']['default'] = $scheduled_first_name;
	    endif;
	    if(!is_null($scheduled_last_name)):
	    	$fields['billing']['billing_last_name']['default'] = $scheduled_last_name;
	    endif;
	    if(!is_null($scheduled_email)):
	    	$fields['billing']['billing_email']['default'] = $scheduled_email;
	    endif;
	    return $fields;
	}
	...
	public static function woocommerce_cart_item_name( $product_title, $cart_item, $cart_item_key ) {
		...
		try {
			...
			$product_title .= '<div class="scheduled-wc-checkout-section"><small>' . ucwords( $appointment->timeslot_text ) . '</small></div>';

			$product_title .= '<div class="scheduled-wc-checkout-section">';

			// Get the Date
			$date_format = get_option('date_format');
			$product_title .= '<small><b>' . __('Date', 'scheduled') . ':</b>&nbsp;' . date_i18n( $date_format, $appointment->timestamp ) . '</small>';

			if ( !empty($appointment->calendar) && !empty($appointment->calendar->calendar_obj) ) {
				// Get the Calendar Name
				$product_title .= '<br><small><b>' . __('Calendar', 'scheduled') . ':</b>&nbsp;' . $appointment->calendar->calendar_obj->name . '</small>';

				// Check for a Booking Agent
				$term_meta = get_option( "taxonomy_{$appointment->calendar->calendar_obj->term_id}" );
	  			$assignee_email = $term_meta['notifications_user_id'];
	  			if ( $assignee_email && ($usr=get_user_by('email', $assignee_email)) ) {
					$product_title .= '<br><small><b>' . __('Booking Agent', 'scheduled') . ':</b>&nbsp;' . $usr->display_name . '</small>';
	  			}
	  		}
	...
	public static function woocommerce_cart_item_thumbnail( $thumbnail, $cart_item ) {
		...
		$app_id_key = SCHEDULED_WC_PLUGIN_PREFIX . 'appointment_id';
		if ( isset($cart_item[$app_id_key]) ) {
			if ( Scheduled_WC_Settings::get_option('enable_thumbnails') === 'enable' ) {
				return $thumbnail;
			} else {
				return false;
			}
