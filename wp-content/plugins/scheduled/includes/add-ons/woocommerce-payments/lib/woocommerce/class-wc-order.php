...
class Scheduled_WC_Order {
	...
	public static function get( $order_id=null ) {
		if ( !is_integer($order_id) ) {
			$message = sprintf( __('%s integer expected when %s given.', 'scheduled'), 'Scheduled_WC_Order::get($order_id)', gettype($order_id) );
	...
	protected function get_appointments() {
		$this->appointments = get_post_meta($this->order_id, '_' . SCHEDULED_WC_PLUGIN_PREFIX . 'order_appointments', true);
		return $this;
	}
}

class Scheduled_WC_Order_Hooks {
	...
	public static function woocommerce_order_remove_appointment( $order_id ) {
		...
		$order = Scheduled_WC_Order::get($order_id);
		...
		foreach ($appointments as $app_id) {
			...
			try {
				do_action('scheduled_appointment_cancelled',$app_id);
	...
	public static function woocommerce_order_complete( $order_id ) {
		...
		foreach ($appointments as $appt_id) {
			...
			$send_upon_completion = Scheduled_WC_Functions::scheduled_disable_confirmation_emails();
			if ( $send_upon_completion ):

				// Add Scheduled WC confirmation email actions
				add_action( 'scheduled_wc_confirmation_email', 'scheduled_mailer', 10, 3 );
				add_action( 'scheduled_wc_admin_confirmation_email', 'scheduled_mailer', 10, 3 );
				...
				if ($email_content && $email_subject):
					...
					do_action( 'scheduled_wc_confirmation_email', $token_replacements['email'], $email_subject, $email_content );
				...
				if ($email_content && $email_subject):
	            	...
	            	$cals = wp_get_object_terms( $appt_id, 'scheduled_custom_calendars' );
					...
					do_action( 'scheduled_wc_admin_confirmation_email', $admin_email, $email_subject, $email_content );

				endif;

				// Remove Scheduled WC confirmation email actions
				remove_action( 'scheduled_wc_confirmation_email', 'scheduled_mailer', 10 );
				remove_action( 'scheduled_wc_admin_confirmation_email', 'scheduled_mailer', 10 );
	...
	public static function woocommerce_validate_order_items( $order_id ) {
		$cart_appointments = Scheduled_WC_Cart::get_cart_appointments();
		...
		foreach ($cart_appointments['ids'] as $app_id) {
			...
			foreach ($appointment->products as $product) {
				...
				if ( !in_array($check_key, $cart_appointments['extended']) ) {
					$message = sprintf( __('Appointment "%s" and cart products do not match. Please make sure that all appointment products are available in the cart.', 'scheduled'), $appointment->timeslot_text );
					throw new Exception( $message );
				}
			}
		}
		...
		foreach ($cart_appointments['ids'] as $app_id) {
			...
			update_post_meta($app_id, '_' . SCHEDULED_WC_PLUGIN_PREFIX . 'appointment_order_id', $order_id);
		}

		if ( $cart_appointments['ids'] ) {
			// link the order with all appointments appointment
			update_post_meta($order_id, '_' . SCHEDULED_WC_PLUGIN_PREFIX . 'order_appointments', $cart_appointments['ids']);
		}
	}
	...
	public static function woocommerce_hidden_order_itemmeta( $hidden_meta ) {
		$hidden_meta[] = SCHEDULED_WC_PLUGIN_PREFIX . 'appointment_timerange';
