...
class Scheduled_WC_Functions {
	...
	public static function get_login_page_info() {
		...
		$query = "SELECT * FROM `{$wpdb->posts}`
					...
					AND `post_content` LIKE '%[scheduled-login]%'";
	...
	public static function get_appointments_page_info() {
		...
		$query = "SELECT * FROM `{$wpdb->posts}`
					WHERE `post_type` = 'page'
					AND `post_content` LIKE '%[scheduled-appointments]%'";
	...
	public static function scheduled_new_appointment_form() {

		if (
			!empty($_POST['action'])
			&& $_POST['action']==='scheduled_new_appointment_form'
			&& !empty($_POST['source'])
			&& $_POST['source']==='scheduled_wc_extension'
		) {
			$booking_form = Scheduled_WC_Fragments::get_path('ajax-loaded/appointment', 'change-date');
	...
	public static function get_custom_fields() {
		...
		if ( $calendar_id ) {
			$custom_fields_option_name = 'scheduled_custom_fields_' . $calendar_id;
	...
	public static function scheduled_custom_field_data( $custom_field_data = array() ) {
		...
		foreach($custom_fields as $key => $field) {
			...
			$product = Scheduled_WC_Product::get($product_id);
			...
			if ( $calendar_id ) {
				...
				if ( $assignee_email && ($usr=get_user_by('email', $assignee_email)) ) {
					$custom_field_data['booking-agent'] = array(
						'label' => esc_attr__('Booking Agent', 'scheduled'),
						'value' => $usr->display_name
					);
				}
			}

			add_filter('scheduled_new_appointment_args', array('Scheduled_WC_Functions', 'scheduled_new_appointment_args_on_create'), 10, 1);
		}
	...
	// filtrates the custom field values before creating a new appointment
	public static function scheduled_prepare_sending_reminder( $send = true, $appt_id = false ) {
		...
		if ( is_array( $appt_id ) ):
			...
			if ( $is_wc_order ):
				$order = new Scheduled_WC_Appointment_Payment_Status( $appt_id );
	...
	// this hook should be called only in Scheduled_WC_Functions::scheduled_custom_field_data() only if product option is available
	public static function scheduled_new_appointment_args_on_create( $post_params=array() ) {

		// just in case, remove the filter
		remove_filter('scheduled_new_appointment_args', array('Scheduled_WC_Functions', 'scheduled_new_appointment_args_on_create'), 10, 1);

		// set post status to awaiting payment
		$post_params['post_status'] = SCHEDULED_WC_PLUGIN_PREFIX . 'awaiting';

		return $post_params;
	}

	// filtered the appointment information before populating to the calendar
	public static function scheduled_appts_array( $appointments_array ) {
		return $appointments_array;
	}

	public static function scheduled_store_appointment_creation_date( $appointment_id=null ) {
		...
		update_post_meta($appointment_id, '_' . SCHEDULED_WC_PLUGIN_PREFIX . 'time_created', $current_time);
		update_post_meta($appointment_id, '_' . SCHEDULED_WC_PLUGIN_PREFIX . 'date_created', date('Y-m-d H:i:s', $current_time));

	}

	public static function scheduled_get_custom_fields_information() {
		$custom_fields = self::get_custom_fields();
	...
	public static function scheduled_store_custom_fields_information( $appointment_id = null ) {
		if ( !$appointment_id ) {
			return;
		}

		$custom_fields = self::scheduled_get_custom_fields_information();
		$i = 0;
		$separator = '--SEP--';
		$meta_key = '_' . SCHEDULED_WC_PLUGIN_PREFIX . 'cfield_';
	...
	public static function scheduled_new_appointment_created( $appointment_id = null ) {

		if ( is_admin() && isset($_POST['scheduled_form_type']) && $_POST['scheduled_form_type'] == 'admin' ):
			return;
		endif;
		...
		try {
			$appointment = Scheduled_WC_Appointment::get($appointment_id);
		} catch (Exception $e) {
			return;
		}
		...
		self::scheduled_store_custom_fields_information($appointment_id);
		$added_to_cart = Scheduled_WC_Cart::add_appointment($appointment_id);
		if ( !$added_to_cart ) {
			return;
		}
	...
	public static function scheduled_shortcode_appointments_buttons( $appointment_id=null ) {
		// add change date and pay buttons
		$edit_button = Scheduled_WC_Fragments::get_path('appointment', 'buttons');
		include($edit_button);
	}

	public static function scheduled_shortcode_appointments_additional_information( $appointment_id=null ) {
		$additional_info = Scheduled_WC_Fragments::get_path('appointment', 'additional-info');
		include($additional_info);
	}

	// Don't show cancel button on appointment with products
	public static function scheduled_shortcode_appointments_allow_cancel($allow_cancel, $app_id) {
		$appointment = Scheduled_WC_Appointment::get(intval($app_id));
		return !$appointment->products;
	}

	// Add "scheduled_wc_awaiting" to the "Pending" status types
	public static function scheduled_admin_pending_post_status($statuses) {
		$statuses = array('draft','scheduled_wc_awaiting');
		return $statuses;
	}

	// remove delete button if order attached
	public static function scheduled_fea_shortcode_appointments_buttons($button_html, $appt_id) {
		$appointment = Scheduled_WC_Appointment::get(intval($appt_id));
		$status = get_post_status(intval($appt_id));
		if ($appointment->order_id):
			$status_class = $status !== 'publish' && $status !== 'future' ? 'pending' : 'approved';
			$button_html = '<div class="scheduled-fea-buttons">';
				$button_html .= ($status_class == 'pending' ? '<button data-appt-id="'.$appt_id.'" class="approve button button-primary">'.__('Approve','scheduled').'</button>' : '');
			$button_html .=	'</div>';
			return $button_html;
		else:
			return $button_html;
	...
	public static function scheduled_button_book_appointment( $label ) {
		...
		$appointment = Scheduled_WC_Appointment::get(intval($_POST['app_id']));
		$current_time = current_time('timestamp');
		...
		return '<span class="button-text" >' . __('Choose New Date', 'scheduled') . '</span></button></span>';
	}

	public static function scheduled_before_creating_appointment() {
		...
		elseif ($customer_type == 'new'):
			$first_name = esc_attr($_POST['scheduled_appt_name']);
			$last_name = ( isset($_POST['scheduled_appt_surname']) && $_POST['scheduled_appt_surname'] ? esc_attr($_POST['scheduled_appt_surname']) : false );
			$email = $_POST['scheduled_appt_email'];
		...
		if ( isset( $email ) ):
			$woocommerce->session->set( 'scheduled_first_name', $first_name );
			$woocommerce->session->set( 'scheduled_last_name', $last_name );
			$woocommerce->session->set( 'scheduled_email', $email );
		endif;
		...
		if (
			(!$action || $action !== 'scheduled_add_appt')
			|| (!$app_id || !intval($app_id))
			|| (!$app_action || $app_action!=='edit')
			|| (!$source || $source!=='scheduled_wc_extension')
		) {
			return;
		}
		...
		remove_action('scheduled_new_appointment_created', array('Scheduled_WC_Functions', 'scheduled_new_appointment_created'), 15, 1);

		// turn the wp_insert_post to act as wp_update_post
		add_filter('scheduled_new_appointment_args', array('Scheduled_WC_Functions', 'scheduled_new_appointment_args_on_date_change'), 10, 1);

		// dont allow to update the metas and the calendar term
		add_filter('scheduled_update_cf_meta_value', array('Scheduled_WC_Functions', 'return_false'), 10);
		add_filter('scheduled_update_appointment_calendar', array('Scheduled_WC_Functions', 'return_false'), 10);

	}

	public static function scheduled_wc_mailer_actions( $mailer_actions ){
		$send_upon_completion = self::scheduled_disable_confirmation_emails();
		if ( $send_upon_completion ):
			$mailer_actions[] = 'scheduled_wc_confirmation_email';
			$mailer_actions[] = 'scheduled_wc_admin_confirmation_email';
			return $mailer_actions;
		endif;
		return $mailer_actions;
	}

	public static function scheduled_disable_confirmation_emails(){
		if ( Scheduled_WC_Settings::get_option('email_confirmations') === 'after_complete' || Scheduled_WC_Settings::get_option('email_confirmations') === false ):
			return apply_filters( 'scheduled_disable_confirmation_emails', true );
		else:
			return apply_filters( 'scheduled_disable_confirmation_emails', false );
		endif;
	}

	public static function remove_default_emails() {

		$scheduled_wc_product = false;

		if (isset($_POST)):
			foreach($_POST as $var => $val):
				$var_parts = explode('---',$var);
				if (isset($var_parts[0]) && $var_parts[0] == 'paid-service-label'):
					$scheduled_wc_product = true;
					break;
				endif;
			endforeach;

			$scheduled_disable_confirmation_emails = self::scheduled_disable_confirmation_emails();
			if ( $scheduled_wc_product && $scheduled_disable_confirmation_emails ):
				remove_action( 'scheduled_confirmation_email', 'scheduled_mailer', 10 );
				remove_action( 'scheduled_admin_confirmation_email', 'scheduled_mailer', 10 );
			endif;
	...
	// see wp_update_post()
	public static function scheduled_new_appointment_args_on_date_change( $post_args ) {
		...
		$app_detailed_obj = Scheduled_WC_Appointment::get(intval($appointment_id));
		$awaiting_payment_status = SCHEDULED_WC_PLUGIN_PREFIX . 'awaiting';
	...
	public static function scheduled_admin_calendar_buttons_before($calendar_id, $appt_id, $status) {
		...
		add_filter('scheduled_admin_show_calendar_buttons', array('Scheduled_WC_Functions', 'return_false'), 10);

		$edit_button = Scheduled_WC_Fragments::get_path('admin-calendar', 'app-buttons');
		include($edit_button);
	}

	public static function scheduled_admin_calendar_buttons_after($calendar_id, $appt_id, $status) {
		remove_filter('scheduled_admin_show_calendar_buttons', array('Scheduled_WC_Functions', 'return_false'), 10);
	}
