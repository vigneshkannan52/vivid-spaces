...
class Scheduled_WC_Cleanup {
	...
	protected function clean() {
		foreach ($this->appointments as $app) {
			...
			if ( !isset($app_meta['_scheduled_wc_appointment_order_id']) || isset($app_meta['_scheduled_wc_appointment_order_id']) && !$app_meta['_scheduled_wc_appointment_order_id'] ):
	...
	protected function appointments_to_clean() {
		...
		$this->appointments = get_posts(array(
			'posts_per_page' => -1,
			'post_type' => SCHEDULED_WC_POST_TYPE,
			'post_status' => array('any'),
			'suppress_filters' => false,
			'_' . SCHEDULED_WC_PLUGIN_PREFIX . 'cleanup' => true
		));
	...
	public function pre_get_posts( $query ) {
		...
		if ( !$query->get( '_' . SCHEDULED_WC_PLUGIN_PREFIX . 'cleanup') ) {
			return $query;
		}
	...
	public function posts_where( $where ) {
		...
		$time_created_meta_key = '_' . SCHEDULED_WC_PLUGIN_PREFIX . 'time_created';
