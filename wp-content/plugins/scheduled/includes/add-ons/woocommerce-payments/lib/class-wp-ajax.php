...
class Scheduled_WC_Ajax {
	...
	private function __construct() {
		...
		foreach ($functions as $ajax_type => $requests) {
			foreach ($requests as $action_value => $function_name) {
				$filter_name = $ajax_type . SCHEDULED_WC_PLUGIN_PREFIX . $action_value;
	...
	public function mark_appointment_as_paid() {
		...
		if (empty($_POST['appt_id']) || !current_user_can('manage_scheduled_options')){
			return;
		}
		...
		if (!$appointment->order_id){
			$_oid = false;
			update_post_meta($appt_id, '_scheduled_wc_appointment_order_id', 'manual');
		} else {
	...
	public function load_product_variations() {
		...
		try {
			...
			$fragment_file = Scheduled_WC_Fragments::get_path('ajax-loaded/product','variations');
			include($fragment_file);
		} catch (Exception $e) {
			$message = __('An error has occur.', 'scheduled');
	...
	public function add_appointment_to_cart() {
		$response = new Scheduled_WC_Response();

		$app_id = (!empty($_POST['app_id']) && $_POST['app_id']) ? intval($_POST['app_id']) : false;
		if ( !$app_id ) {
			$response->add_message(__('Appointment ID is not defined.', 'scheduled'));
			$response->create();
		}
		...
		try {
			...
			Scheduled_WC_Cart::add_appointment($app_id);
		} catch (Exception $e) {
			$response->add_message($e->getMessage());
			$response->create();
		}

		$response->add_message(__('Appointment has been added to the cart', 'scheduled'));
