...
function scheduled_profile_content_fea_appointments(){
	echo do_shortcode('[scheduled-fea-appointments remove_wrapper=1]');	
}

function scheduled_profile_content_fea_history(){
	echo do_shortcode('[scheduled-fea-appointments remove_wrapper=1 historic=1]');	
}

function scheduled_profile_content_fea_pending(){
	echo do_shortcode('[scheduled-fea-appointments remove_wrapper=1 pending=1]');	
}
	
function scheduled_agent_appointments($user_id,$only_count = false,$time_format = false,$date_format = false,$calendar_ids = array(),$pending = false,$historic = false){
	...
	if ($pending):
		$statuses = apply_filters('scheduled_admin_pending_post_status',array('draft'));
	else:
		$statuses = apply_filters('scheduled_admin_approved_post_status',array('publish','future'));
	endif;
