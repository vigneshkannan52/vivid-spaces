...
class Scheduled_FEA_Shortcodes {

	function __construct(){
		add_shortcode('scheduled-fea-appointments', array($this, 'scheduled_fea_appointments_shortcode') );
	}

	/* FEA APPOINTMENTS SHORTCODE */
	public function scheduled_fea_appointments_shortcode($atts = null, $content = null) {
		...
		if (is_user_logged_in() && current_user_can('edit_scheduled_appts')):
			...
			$calendars = get_terms('scheduled_custom_calendars',array('order_by' => 'slug','hide_empty' => false));
			$calendar_ids = array();
			...
			if (!empty($calendars)):
				foreach($calendars as $calendar):
					...
					if ($scheduled_current_user->user_email == $term_meta['notifications_user_id']):
			...
			$appointments_array = scheduled_agent_appointments($my_id,false,$time_format,$date_format,$calendar_ids,$pending,$historic);
			...
			if (!isset($atts['remove_wrapper'])): echo '<div id="scheduled-profile-page" class="scheduled-shortcode">'; endif;
				...
				echo '<div class="scheduled-profile-appt-list scheduled-fea-appt-list">';
				
					$scheduled_light_color = get_option('scheduled_light_color','#44535B');
					$scheduled_button_color = get_option('scheduled_button_color','#56C477');
				
					echo '<style type="text/css">';
						echo "body #scheduled-profile-page .scheduled-profile-appt-list button.button-primary { background:$scheduled_button_color; border-color:$scheduled_button_color; color:#fff; }\n";
						echo "body #scheduled-profile-page .scheduled-profile-appt-list button.button-primary:hover { background:$scheduled_light_color; border-color:$scheduled_light_color; }";
						echo "body #scheduled-profile-page .scheduled-profile-appt-list .appt-block .scheduled-wc_status-text.paid { color:$scheduled_button_color; }";
					echo '</style>';

					if ($pending):
						if ($total_appts):
							echo '<h4>'.sprintf(_n('%s Pending Appointment','%s Pending Appointments',$total_appts,'scheduled'),'<span class="count">'.number_format($total_appts).'</span>').'</h4>';
						else:
							echo '<p class="scheduled-no-margin">'.__('No pending appointments.','scheduled').'</p>';
						endif;
					elseif ($historic):
						if ($total_appts):
							echo '<h4>'.sprintf(_n('%s Past Appointment','%s Past Appointments',$total_appts,'scheduled'),'<span class="count">'.number_format($total_appts).'</span>').'</h4>';
						else:
							echo '<p class="scheduled-no-margin">'.__('No past appointments.','scheduled').'</p>';
						endif;	
					else:
						if ($total_appts):
							echo '<h4>'.sprintf(_n('%s Upcoming Appointment','%s Upcoming Appointments',$total_appts,'scheduled'),'<span class="count">'.number_format($total_appts).'</span>').'</h4>';
						else:
							echo '<p class="scheduled-no-margin">'.__('No upcoming appointments.','scheduled').'</p>';
						endif;
					endif;
					...
					foreach($appointments_array as $appt):
						...
						if ($date_display == $today){
							$date_display = __('Today','scheduled');
							$day_name = '';
						} else {
						...
						if ($timeslots[0] == '0000' && $timeslots[1] == '2400'):
							$timeslotText = __('All day','scheduled');
							$google_date_startend_end = date('Ymd',strtotime(date('Y-m-d',$appt['timestamp']) . '+ 1 Day'));
							$google_time_end = '0000';
						else :
							$timeslotText = (!get_option('scheduled_hide_end_times') ? __('from','scheduled').' ' : __('at','scheduled').' ') . $time_start . (!get_option('scheduled_hide_end_times') ? ' ' . __('to','scheduled').' '.$time_end : '');
							$google_date_startend_end = $google_date_startend;
						endif;

						$status = ($appt['status'] !== 'publish' && $appt['status'] !== 'future' ? __('pending','scheduled') : __('approved','scheduled'));
						$status_class = $appt['status'] !== 'publish' && $appt['status'] !== 'future' ? 'pending' : 'approved';
						
						echo '<div class="appt-block scheduledClearFix" data-appt-id="'.$appt['post_id'].'">';
						
							$default_button_html = '<div class="scheduled-fea-buttons">';
								$default_button_html .= '<a href="#" class="delete"'.($calendar_id ? ' data-calendar-id="'.$calendar_id.'"' : '').'><i class="fa-solid fa-xmark"></i></a>';
								$default_button_html .= ($status_class == 'pending' ? '<button data-appt-id="'.$appt['post_id'].'" class="approve button button-primary">'.__('Approve','scheduled').'</button>' : '');
							$default_button_html .=	'</div>';
						
							$default_button_html = apply_filters('scheduled_fea_shortcode_appointments_buttons', $default_button_html, $appt['post_id']);
							...
							if (!isset($appt['guest_name'])):
								...
								if (isset($user_info->ID)):
									...
									$user_display = __('(this user no longer exists)','scheduled');
								endif;
							else :
							...
							echo $user_display;
							echo '<br>';
							if ($late_date > $date_to_compare): echo '<span class="late-appt">' . __('This appointment has passed.','scheduled') . '</span><br>'; endif;
							...
							echo ($cf_meta_value ? '<br><i class="fa-solid fa-circle-info"></i><a href="#" class="scheduled-show-cf">'.__('Additional information','scheduled').'</a><div class="cf-meta-values-hidden">'.$cf_meta_value.'</div>' : '');
			...
			return '<p>'.__('Please log in to view your upcoming appointments.','scheduled').'</p>';
...
$scheduled_fea_shortcodes = new Scheduled_FEA_Shortcodes;