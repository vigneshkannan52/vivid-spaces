...
if ( !isset($_GET['sh']) || isset($_GET['sh']) && $_GET['sh'] != SCHEDULEDICAL_SECURE_HASH )
...
header('Content-Disposition: attachment; filename=appointment-feed-'.SCHEDULED_VERSION.'.ics');
...
PRODID:-//getscheduled.io//Scheduled Calendar<?php echo "\r\n"; ?>
	...
	foreach($appts_in_this_timeslot as $appt_id):
		...
		if (!$guest_name):
			...
			if ($user_id):
				...
				$display_name = scheduled_get_name($user_id);
				if ( !empty($user_info) ):
					$email = $user_info->user_email;
				else:
					$display_name = esc_html__('[No name]','scheduled');
					$email = esc_html__('[No email]','scheduled');
				endif;
			else:
				$display_name = esc_html__('[No name]','scheduled');
				$email = esc_html__('[No email]','scheduled');
...
?>SUMMARY:<?php echo apply_filters( 'scheduled_calendar_feed_display_name', $display_name, $appt_id ); ?><?php echo "\r\n"; ?>
<?php echo ( $description ? "DESCRIPTION:" . $description . "\r\n" : "" ); ?>
UID:scheduled-appointment-<?php echo $appt_id; ?><?php echo "\r\n"; ?>
END:VEVENT<?php echo "\r\n"; ?>
<?php
