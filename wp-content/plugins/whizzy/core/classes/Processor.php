 ...
 * @author     FOXTHEMES
 */
class WhizzyProcessorImpl implements WhizzyProcessor {
	...
	/** @var WhizzyMeta current data; including submitted data */
	protected $data = null;
		...
		try {
			...
			if ( $this->form_was_submitted() ) {
				...
				if ( empty( $errors ) ) {
					...
					$new_option = apply_filters( 'whizzy_before_update_setting_fields', array_merge( $current_values, $input ) );
					update_option( $option_key, $new_option );
					$this->data = whizzy::instance( 'WhizzyMeta', $input );
	...
	protected function load_data_from_database( $option_key ) {
		...
		$this->data = whizzy::instance( 'WhizzyMeta', $dbconfig );
	}
	...
	protected function validate_input( $input ) {
		$validator = whizzy::instance( 'WhizzyValidator', $this->meta, $this->fields );
	...
	/**
	 * @return WhizzyMeta current data (influenced by user submitted data)
	 */
	function data() {
		if ( $this->status === null ) {
			$this->run();
